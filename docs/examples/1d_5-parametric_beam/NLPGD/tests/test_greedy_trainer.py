...
def test_frozen_modes_are_bitwise_unchanged_by_later_stages(problem):
    ...
    #
    # renormalise=False is not incidental here, and it is the one place this file
    # differs from its CP twin. PolynomialNLPGD.renormalise() is a real operation
    # (CPPGD's is a no-op), and GreedyTrainer.prepare_stage calls it at every
    # stage boundary: it rescales the monoms of EVERY active mode, frozen ones
    # included. That is exact and field-preserving -- the gauge orbit's defining
    # property -- but it is emphatically not bitwise. So "frozen" has to be
    # tested with the gauge fix off; what holds with it on is the weaker and
    # separately-tested "the frozen mode's contribution to the field is
    # unchanged".
    trainer = make_trainer(
        problem, enrichment_criterion=MaxStages(1), renormalise=False
    )
    trainer.enrich()
...
def test_the_gauge_fix_preserves_a_frozen_modes_contribution(beam5p, problem):
    """The property that survives ``renormalise=True``, which is the default.

    With the gauge fix on, a frozen mode's stored parameters DO change (see the
    test above). What must not change is what they represent: ``renormalise``
    moves amplitude between axes along a direction the field is invariant under.
    Measured on the mode's own contribution, evaluated with every other mode
    switched off by construction -- a rank-1 evaluation at a grid of points.
    """
    trainer = make_trainer(problem, enrichment_criterion=MaxStages(1))
    trainer.enrich()

    points = torch.stack(
        [torch.linspace(*beam5p.AXIS_BOUNDS[name], 9) for name in beam5p.AXIS_ORDER],
        dim=1,
    )
    before = problem.pgd.evaluate(points).clone()

    problem.pgd.renormalise()

    # float32 and a product of five factors: exact equality is not on offer, but
    # the field must not move beyond round-off.
    assert torch.allclose(problem.pgd.evaluate(points), before, rtol=1e-5, atol=1e-5)
