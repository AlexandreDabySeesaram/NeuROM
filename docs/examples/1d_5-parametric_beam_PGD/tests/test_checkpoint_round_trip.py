...
def test_the_checkpoint_path_is_one_file_per_strategy(beam5p):
    ...
    greedy = beam5p.checkpoint_path(GreedyTrainer)
    simultaneous = beam5p.checkpoint_path(SimultaneousTrainer)

    # Two strategies compared against the same reference must not overwrite
    # each other's trained model.
    assert greedy != simultaneous
    assert greedy.name == "pgd5_greedy.pt"
    assert simultaneous.name == "pgd5_simultaneous.pt"
