...
if len(Model_du.constit_BC_node_IDs)>0:
    ...
    constit_point_coord = [torch.tensor(Model_u.coordinates[i]) for i in constit_point_IDs]
    constit_cell_IDs_u = torch.tensor([torch.tensor(Domain_mesh_u.GetCellIds(i),dtype=torch.int)[0] for i in constit_point_coord])
    constit_point_coord = torch.tensor(torch.cat(constit_point_coord),requires_grad=True)
else:
    constit_cell_IDs_u = []
    constit_point_coord = []
...
num_sol_name = "Rectangle/Rectangle_order_1_2.5_order2_displacement.npy"
