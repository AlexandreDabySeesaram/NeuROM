...
class NeuROM(nn.Module):
    ...
    def __init__(self, mesh, ParametersList, config, n_modes_ini = 1, n_modes_max = 100):
        ...
        self.register_buffer('n_modes_truncated',torch.min(torch.tensor(self.n_modes),torch.tensor(n_modes_ini))) 
            ...
            case '3':
                match self.n_para:
                    ...
                    case 4:
                        ...
                        out = torch.einsum('xyk,kj,kp,kl,kq->xyjplq',u_i,P1,P2,P3,P4)
                    case 5:
                        Space_modes = []
                        for i in range(self.n_modes_truncated):
                            if self.Space_modes[i].IdStored and x.shape == self.Space_modes[i].Stored_ID["coordinates"].shape and x.device == self.Space_modes[i].Stored_ID["coordinates"].device:
