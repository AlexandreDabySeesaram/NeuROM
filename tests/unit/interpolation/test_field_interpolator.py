...
class TestFieldInterpolator:
    ...
    def test_field_interpolator_with_trainable_field(self, trainable_field):
        """Test the FieldInterpolator class with a TrainableField
        ...
        for val in values:
            xi = val["xi"]
            expected = val["expected"]
            interp = field_interp.at_reference(xi).detach()
            assert interp == pytest.approx(expected, rel=self.relative_tolerance), (
                "xi = ",
                xi,
            )
